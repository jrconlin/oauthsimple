...
class OAuthSimple:
    from OAuthSimple import OAuthSimpleException
    _secrets={}
    _parameters={}
    _default_signature_method="HMAC-SHA1"
    _action="GET"
    _nonce_characters='0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
    sbs=""
    _path=""
    ...
    def setParameters(self,parameters={}):
        if parameters:
            ...
            if not self._parameters:
                ...
                self._parameters = self._arrayMerge(self._parameters,parameters)
    ...
    def setAction(self, action):
        ...
        if re.match('[^A-Z]',action):
            raise OAuthSimpleException('Invalid action specified for OAuthSimple.setAction');
        self._action = action;
        return self;
    
    def signatures(self,signatures):
        if signatures and (type(signatures) != type({})):
            raise OAuthSimpleException('Must pass Dict to OAuthSimple.signatures')
        ...
        if self._secrets.get('oauth_token') and not self._secrets.get('oauth_secret'):
            raise OAuthSimpleException('Missing oauth_secret for supplied oauth_token')
    ...
    def sign(self,args={}):
        ...
        self._parameters['oauth_signature'] = self._generateSignature(normParamString)
        return {'parameters':self._parameters,
                'signature':self._oauthEscape(self._parameters['oauth_signature']),
                'signed_url':self._path+'?'+self._normalizeParameters(),
    ...
    def getHeaderString(self,args={}):
        ...
        for pName in self._parameters:
            ...
            if type(pValue) == type([]):
                for val in pValue:
                    result.append(pName + '="' + self._oauthEscape(val) + '"')
            else:
                result.append(pName + '="' + self._oauthEscape(str(pValue)) + '"');
        return 'OAuth '+', '.join(result);
    ...
    def _getApiKey(self):
        if not self._parameters.has_key('oauth_consumer_key'):
            if not self._secrets.get('oauth_consumer_key'):
                raise OAuthSimpleException('No oauth_consumer_key set')
            self._parameters['oauth_consumer_key']=self._secrets.get('oauth_consumer_key')
        return self._parameters.get('oauth_consumer_key');
        
    def _getAccessToken(self):
        if not self._secrets.has_key('oauth_secret'):
            return ''
        if not self._parameters.has_key('oauth_access_token'):
            if not self._secrets.get('oauth_access_token'):
                raise OAuthSimpleException('No oauth_access_token set')
            self._parameters['oauth_access_token'] = self._secrets.get('oauth_access_token')
        ...
        for paramName in pKeys:
            ...
            if type(paramValue) == type([]):
                ...
                for value in paramValue:
                    elements.append(self._oauthEscape(paramName)+'='+self._oauthEscape(value))
                next
            else:
                elements.append(self._oauthEscape(paramName)+'='+self._oauthEscape(str(paramValue)))
